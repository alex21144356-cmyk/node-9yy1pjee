...
app.use(
  session({
    secret: process.env.SESSION_SECRET || 'secreto_ligero',
    resave: false,
...
app.post('/api/armas', async (req, res) => {
  ...
  if (!nombre || !tipo || !dano) return res.status(400).json({ error: 'Campos incompletos' });
...
app.post('/api/equipamiento', async (req, res) => {
  ...
  if (!req.session || !req.session.username) return res.status(401).json({ error: 'Inicia sesion primero' });
